...
const getProducts = async (req, res, next) => {
    ...
    if (req.query.populateCategory) {
      productsQuery.populate("category");
		...
		return res.status(200).json(createResponse(productsQuery, 'Products get successfully', false));
	} catch (err) {
		next(err);
	}
};
...
const getSingleProduct = async (req, res, next) => {
	try {
		...
		// check condition
		if (!product) {
			res.status(404).json(createResponse(null, 'Product not found', true, false));
		}
		return res.status(200).json(createResponse(product, 'Product get successfully', false));
	} catch (error) {
		next(error);
	}
};
...
const updateProduct = async (req, res, next) => {
	try {
		...
		return res.json(createResponse(product, 'Product updated successfully', false));
	} catch (error) {
		next(error);
	}
};
