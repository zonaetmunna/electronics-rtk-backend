...
const getCategories = async (req, res, next) => {
	try {
		...
		const excludeFields = ['search', 'category', 'sort', 'page', 'limit', 'minPrice', 'maxPrice'];
...
const getSingleCategory = async (req, res, next) => {
	try {
		const { id } = req.params;
		const category = await Category.findOne({ _id: id }).populate('products').exec();
		if (!category) {
			return res.json(createResponse(null, 'Category not found', true));
		}
		return res.json(createResponse(category, 'Category fetched successfully', false));
	} catch (error) {
		next(error);
	}
};
...
const deleteCategory = async (req, res, next) => {
	try {
		...
		return res.json(createResponse(null, 'Category deleted successfully', false));
	} catch (error) {
		next(error);
	}
};
