...
const signupUser = async (req, res, next) => {
	try {
		...
		if (body.email) {
			...
			if (existingUser) {
				return res.status(400).json(createResponse(null, 'Email already exists!', false));
			}
		}
		// hash the password
...
const googleSignin = async (req, res, next) => {
	try {
		...
		if (body.email) {
			...
			if (existingUser) {
				return res.status(201).json(createResponse(existingUser, 'login successful!', false));
			}
		}
		...
		await user.save();
		console.log(user);
		return res.status(201).json(createResponse(user, 'Registration successful', false));
	} catch (error) {
		next(error);
	}
};
